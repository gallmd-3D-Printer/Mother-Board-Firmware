...
public:
	...
	GPIO(){
                ...
                //Enable RCC_MDG_AHB1 Clock
                if(gpio_bank == GPIOxBaseRegisters::GPIO_A){
              
                  bankEnable = RCC_MDG::AHB1ENR::GPIOAEN;
                  
                }   

                if(gpio_bank == GPIOxBaseRegisters::GPIO_B){
          
                  bankEnable = RCC_MDG::AHB1ENR::GPIOBEN;

                }   

                if(gpio_bank == GPIOxBaseRegisters::GPIO_C){
         
                  bankEnable = RCC_MDG::AHB1ENR::GPIOCEN;

                } 
                if(gpio_bank == GPIOxBaseRegisters::GPIO_D){
                
                  bankEnable = RCC_MDG::AHB1ENR::GPIODEN;

                }              
                if(gpio_bank == GPIOxBaseRegisters::GPIO_E){
                  
                  bankEnable = RCC_MDG::AHB1ENR::GPIOEEN;
          
                }  
                if(gpio_bank == GPIOxBaseRegisters::GPIO_F){
             
                  bankEnable = RCC_MDG::AHB1ENR::GPIOFEN;

                }                                           
                if(gpio_bank == GPIOxBaseRegisters::GPIO_G){
               
                  bankEnable = RCC_MDG::AHB1ENR::GPIOGEN;

                } 
                if(gpio_bank == GPIOxBaseRegisters::GPIO_H){

                  bankEnable = RCC_MDG::AHB1ENR::GPIOHEN;
                }
                if(gpio_bank == GPIOxBaseRegisters::GPIO_I){

                  bankEnable = RCC_MDG::AHB1ENR::GPIOIEN;

                }    
                  dynamic_access<RCC_MDG::RCCRegisterType2, 
                  RCC_MDG::RCCRegisterType2>::reg_or( 
                  RCC_MDG::BaseRegisters::RCC_MDG + RCC_MDG::AHB1ENR::registerOffset, 
                  bankEnable);
...
//		dynamic_access<GPIOxRegisterType, uint32_t>::reg_or(GPIOxModeRegister, tempMask);
	...
	void clear() const{
        ...
        bool readPin() const{
          ...
          inputRegister = reg_access<GPIOxRegisterType, GPIOxRegisterType, GPIOxInputDataRegister, 0x00>::reg_get();
          if(inputRegister & bitPosition){
            return true;
          }else{
          return  false;
          }
...
private:
	...
	static constexpr GPIOxRegisterType bitPosition = (1UL << (pinNumber -1));
        RCC_MDG::RCCRegisterType2 bankEnable;
